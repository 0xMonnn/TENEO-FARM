...
console.log(
  chalk.cyan.bold(
    `                JOIN TELEGRAM @AirdropSyncReborn                `
  )
);
...
process.on("SIGINT", () => {
  console.log(
    "Received SIGINT. Stopping pinging and disconnecting WebSocket..."
  );
  stopPinging();
  disconnectWebSocket();
  process.exit(0);
    ...
    rl.question("Enter your password: ", (password) => {
      rl.question("Enter invited_by code: ", async (invitedBy) => {
        try {
          ...
          console.log(
            "Registration successful Please Confirm your email at :",
            email
          );
        } catch (error) {
          console.error(
  ...
  rl.question("Do you want to use a proxy? (y/n): ", async (useProxy) => {
    ...
    if (useProxy.toLowerCase() === "y") {
      proxy = await new Promise((resolve) => {
        rl.question(
          "Please enter your proxy URL (e.g., http://username:password@host:port): ",
          (inputProxy) => {
            resolve(inputProxy);
          }
        );
          ...
          switch (option) {
            case "1":
              fs.unlink("localStorage.json", (err) => {
                if (err) {
                  console.error(
                    "Error deleting localStorage.json:",
                    err.message
                  );
                } else {
